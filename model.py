...
class Cnns(nn.Module):
    def __init__(self,
                 encoder_channel_size=[64, 64, 64, 128, 128, 128, 256, 256, 256],
                 kernel_size=3,
                 padding=1,
                 dropout=0.2,
                 frame_len=512,
    ...
    def wave2spec(self, wave_out):
        spec_out = torch.stft(wave_out.squeeze(), n_fft=self.n_fft, hop_length=self.frame_shift,
                              win_length=self.frame_len, window=torch.hann_window(self.frame_len).cuda(self.device),
                              center=False).cuda(self.device)
        # mag = sqrt(real_part ** 2 + imag_part ** 2)
        mag_out = torch.sqrt(torch.sum(spec_out ** 2, -1))
        return mag_out
    ...
    def out2mag(self, outputs):
        ...
        mag = torch.zeros(outputs.shape[0], n_frames, self.n_fft // 2 + 1).cuda(self.device)
        for sequence in range(outputs.shape[0]):
            for frame_idx in range(n_frames):
                ...
                frame = frame * torch.hann_window(self.frame_len).cuda(self.device)
                ...
                mag[sequence, frame_idx, :] = torch.sqrt(real_stft ** 2 + imag_stft ** 2 + 1e-12)  # get magnitude
...
if __name__ == "__main__":
    device = 'cuda:0'
    # in_features = torch.randn(32, 1, 2048).cuda(device)
